...
class SpeedtestRunner:
    ...
    def check_librespeed(self) -> bool:
        ...
        try:
            result = subprocess.run(['librespeed-cli', '--version'], 
    ...
    def run_speedtest(self) -> Optional[Dict]:
        ...
        for attempt in range(self.retry_count):
            try:
                logger.info(f"Running speed test (attempt {attempt + 1}/{self.retry_count})")
    ...
    def _parse_result(self, output: str) -> Optional[Dict]:
        ...
        try:
            ...
            if 'server' in data:
                server = data['server']
                measurement['server_name'] = f"{server.get('name', 'Unknown')} ({server.get('country', 'Unknown')})"
    ...
    def run_measurement(self, test_type: str = 'bandwidth') -> bool:
        ...
        if measurement:
            try:
                ...
                logger.info(f"Stored measurement {measurement_id}: {measurement['test_type']} - "
...
def main():
    ...
    parser.add_argument('--type', choices=['bandwidth', 'latency', 'both'], 
                       default='bandwidth', help='Type of test to run')
